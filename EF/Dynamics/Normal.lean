...
def Normal.rename
  ...
  | .neutral neut => by dsimp only [Expr.rename]; exact Normal.neutral (Neutral.rename ρ neut)
  | .value val => by dsimp only [Expr.rename]; exact Normal.value (Value.rename ρ val)
