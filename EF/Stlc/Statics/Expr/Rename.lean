...
def extᵣ
  ...
  | [], _ => by dsimp only [List.nil_append]; exact ρ
  | τ :: tys, _ => by
...
theorem shift_commute
  ...
  let lemma {τ' : Ty} (a : (Γ ∋ τ')) : @Eq (τ :: Δ ∋ τ') (((ext ρ) ∘ shift) a) ((shift ∘ ρ) a) := rfl
...
theorem shift_swap
  ...
  let lemma
    ...
    | .here => by dsimp only [swap, shift]; rfl
    | .there a => by dsimp only [swap, shift]; rfl
...
def rename
  ...
  | .abstraction e => Expr.abstraction (rename ρ.ext e)
  | .application e₁ e₂ => Expr.application (rename ρ e₁) (rename ρ e₂)
...
abbrev shift {Γ : Context} {τ τ' : Ty} (e : Γ ⊢ τ') : (τ :: Γ) ⊢ τ' := rename Rename.shift e

abbrev swap {Γ : Context} {τ₁ τ₂ τ₃ : Ty} (e : (τ₂ :: τ₁ :: Γ) ⊢ τ₃) : (τ₁ :: τ₂ :: Γ) ⊢ τ₃ := rename Rename.swap e
