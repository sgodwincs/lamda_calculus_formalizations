...
def erase {Γ : Context} {τ : Ty} : (Γ ⊢ τ) → Bound.Expr Γ.length
  ...
  | .zero => Bound.Expr.zero
  | .succ e => Bound.Expr.succ (erase e)
  | .recursor e e₀ eₛ => Bound.Expr.recursor (erase e) (erase e₀) (erase eₛ)
...
def infer (Γ : Context) : (e : Bound.Expr Γ.length) → Option (Σ τ : Ty, Γ ⊢ τ)
  | .var i =>
      ...
      some ⟨_, Expr.var a⟩
  | .zero => some ⟨_, Expr.zero⟩
  | .succ e => do
      let e' ← check Γ e Ty.nat
      some ⟨_, Expr.succ e'⟩
  | .recursor e e₀ eₛ => do
      let e' ← check Γ e Ty.nat
      let ⟨τ, e₀'⟩ ← infer Γ e₀
      let eₛ' ← check (τ :: Ty.nat :: Γ) eₛ τ
      some ⟨_, Expr.recursor e' e₀' eₛ'⟩
  | .abstraction τ e => do
      let ⟨τ', e'⟩ ← infer (τ :: Γ) e
...
def check (Γ : Context) : (e : Bound.Expr Γ.length) → (τ : Ty) → Option (Γ ⊢ τ)
  | .var i, τ =>
      ...
      | isFalse _ => none
  | .zero, τ =>
      match decEq τ Ty.nat with
      | isTrue eq => by subst_vars; exact some Expr.zero
      | isFalse _ => none
  | .succ e, τ =>
      match decEq τ Ty.nat with
      | isTrue eq => do
          let e' ← check Γ e Ty.nat
          some (by subst_vars; exact Expr.succ e')
      | isFalse _ => none
  | .recursor e e₀ eₛ, τ => do
      let e' ← check Γ e Ty.nat
      let e₀' ← check Γ e₀ τ
      let eₛ' ← check (τ :: Ty.nat :: Γ) eₛ τ
      some (Expr.recursor e' e₀' eₛ')
...
def infer_sound {Γ : Context} {τ : Ty} : (e : Γ ⊢ τ) → infer Γ e.erase = some ⟨τ, e⟩
  ...
  | .zero => by simp only [erase, infer]
  | .succ e => by simp only [erase, infer, check_sound e, Bind.bind, Option.bind]
  | .recursor e e₀ eₛ => by simp only [erase, infer, check_sound e, infer_sound e₀, check_sound eₛ, Bind.bind, Option.bind]
...
def check_sound {Γ : Context} {τ : Ty} : (t : Γ ⊢ τ) → check Γ t.erase τ = some t
  ...
  | .zero => by
      simp only [erase, check]
      match decEq Ty.nat Ty.nat with
      | isTrue _ => simp only
  ...
  | .succ e => by
      ...
      | isFalse _ => contradiction
  | .recursor e e₀ eₛ => by simp only [erase, check, check_sound e, check_sound e₀, check_sound eₛ, Bind.bind, Option.bind]
...
by
  intro ⟨e, eq⟩
  match e with
  | .var a | .zero => unfold erase infer at *; subst_vars; contradiction
  | .succ e =>
  ...
  | .recursor e e₀ eₛ =>
      unfold erase infer at *
      subst_vars
      simp only [check_sound e, infer_sound e₀, check_sound eₛ, Bind.bind, Option.bind] at h
  | .abstraction e =>
      unfold erase infer at *
      subst_vars
...
by
  ...
  | .var a =>
      ...
      | isFalse _ => contradiction
  | .zero => unfold erase check at *; subst eq; contradiction
  | .succ e =>
      unfold erase check at *
      subst_vars
      simp only at h
      match h' : decEq Ty.nat Ty.nat with
      | isTrue _ => subst_vars; simp_all only [check_sound e, Bind.bind, Option.bind]
      | isFalse _ => contradiction
  | .recursor e e₀ eₛ =>
      unfold erase check at *
      subst_vars
      simp only [check_sound e, check_sound e₀, check_sound eₛ, Bind.bind, Option.bind] at h
