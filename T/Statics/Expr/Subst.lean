...
def subst {Γ Δ : Context} {τ : Ty} (σ : Subst Γ Δ) : (Γ ⊢ τ) → Δ ⊢ τ
  | .var a => σ a
  | .zero => Expr.zero
  | .succ e => Expr.succ (e.subst σ)
  | .recursor e e₀ eₛ => Expr.recursor (e.subst σ) (e₀.subst σ) (eₛ.subst (Subst.exts σ.exts))
...
theorem rename_subst_subst {Γ Δ : Context} {τ : Ty} (ρ : Rename Γ Δ) : @Expr.rename _ _ τ ρ = Expr.subst (subst ρ) :=
  ...
  | .var _ | .zero => rfl
  | .succ e => by simp only [Expr.rename, Expr.subst, lemma ρ e]
  | .recursor e e₀ eₛ => by simp only [Expr.rename, Expr.subst, lemma ρ e, lemma ρ e₀, lemma (Rename.ext ρ.ext) eₛ, subst_ext]
...
theorem subst_id {Γ : Context} {τ : Ty} : @Eq ((Γ ⊢ τ) → Γ ⊢ τ) (⟪ ids⟫ ) id :=
  ...
  | .var _ | .zero => rfl
  | .succ e => by simp [Expr.subst, lemma e, id_eq]
  | .recursor e e₀ eₛ => by simp only [Expr.subst, lemma e, lemma e₀, lemma eₛ, id_eq, exts_cons_shift, subst_id_left, zero_shift]
...
@[simp]
theorem subst_recursor
  {Γ Δ : Context} {τ : Ty}
  (σ : Subst Γ Δ)
  (e : Γ ⊢ Ty.nat)
  (e₀ : Γ ⊢ τ)
  (eₛ : τ :: Ty.nat :: Γ ⊢ τ) :
  (⟪ σ ⟫) (Expr.recursor e e₀ eₛ) = (Expr.recursor ((⟪ σ ⟫) e) ((⟪ σ ⟫) e₀) ((⟪ Subst.exts σ.exts ⟫) eₛ)) :=
by
  simp only [Expr.subst, exts_cons_shift]

@[simp]
theorem subst_succ {Γ Δ : Context} (σ : Subst Γ Δ) (e : Γ ⊢ Ty.nat) : (⟪ σ ⟫) (Expr.succ e) = (Expr.succ ((⟪ σ ⟫) e)) := rfl
...
theorem subst_shift_commuteᵣ
  ...
  | τ, .var a => by
      ...
      exact lemma τs a
  | _, .zero => rfl
  | _, .succ e => by simp only [Expr.rename, Expr.subst, subst_shift_commuteᵣ τs e]
  | _, .recursor e e₀ eₛ => by
      rename_i τ
      simp only [Expr.rename, Expr.subst, subst_shift_commuteᵣ τs e, subst_shift_commuteᵣ τs e₀]
      have := @subst_shift_commuteᵣ _ _ τ τ' σ (τ :: Ty.nat :: τs) eₛ
      congr
  | _, .abstraction e => by
      rename_i τ₁ τ₂
      dsimp only [Expr.rename, Expr.subst]
...
theorem subst_subst
  ...
  (⟪ σ' ⟫) ((⟪ σ ⟫) e) = (⟪ σ ⨟ σ' ⟫) e
  | .var _ | .zero => rfl
  | .succ e => by simp only [Expr.subst, subst_subst σ σ' e]
  | .recursor e e₀ eₛ => by
      have eq := extsᵣ_seq_dist σ σ' [τ, Ty.nat]
      simp only [extsᵣ, id_eq] at eq
      simp only [
        Expr.subst,
        subst_subst σ σ' e,
        subst_subst σ σ' e₀,
        subst_subst (Subst.exts σ.exts) (Subst.exts σ'.exts) eₛ,
        eq
      ]
