...
def Normal.renamed
  ...
  match norm with
  | .neutral neut => by simp only [Typing.rename]; exact Normal.neutral (Neutral.renamed ρ neut)
  | .value val => by simp only [Typing.rename]; exact Normal.value (Value.renamed ρ val)
